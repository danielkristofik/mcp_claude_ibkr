#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# install-linux.sh — Linux (Ubuntu/Debian) installer for claude-tws-connect
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
find_python() {
    ...
    # 2) Check common versioned binaries
    for py in python3.{14,13,12,11,10}; do
    ...
    # 3) Not found — offer apt install
    warn "Python ${REQUIRED_PYTHON_MAJOR}.${REQUIRED_PYTHON_MINOR}+ not found."
    echo ""

    if command -v apt &>/dev/null; then
        read -rp "$(echo -e "${YELLOW}Install Python 3 via apt? [Y/n] ${NC}")" answer
        answer="${answer:-Y}"
        if [[ "$answer" =~ ^[Yy]$ ]]; then
            info "Running: sudo apt update && sudo apt install -y python3 python3-venv python3-pip"
            sudo apt update
            ...
            error "Python install succeeded but version ${REQUIRED_PYTHON_MAJOR}.${REQUIRED_PYTHON_MINOR}+ not available. Please install manually."
            exit 1
        else
        ...
        error "apt not found. Please install Python ${REQUIRED_PYTHON_MAJOR}.${REQUIRED_PYTHON_MINOR}+ manually:"
        echo "  Option A: sudo apt install python3 python3-venv python3-pip"
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║  claude-tws-connect installer (Linux)        ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${NC}"
